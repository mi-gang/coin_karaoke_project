<%@page import="com.oopsw.model.vo.UserVO" %>
	<%@page import="com.oopsw.service.UserService" %>
		<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
			<% session=request.getSession(); String userId=(String)session.getAttribute("userId"); UserVO user=new
				...
				<body>
					<div id="mobileContainer">
						<!--헤더-->
						<header>
							<img src="img/left arrow.svg" alt="이전 페이지 이동"> <span>회원정보
								수정</span>
						</header>
						<!-- 컨텐츠 컨테이너 -->
						<div id="container">
							<form action="controller?cmd=login" method="post">
								<div id="inputField">
									...
									<div>
										<input name="userId" id="userId" placeholder="<%= user.getUserId() %>"
											value="<%= user.getUserId() %>" readonly>
									</div>
									<div>
										<input name="nickname" id="nickname"
											data-original-id="<%= user.getNickname() %>"
											value="<%= user.getNickname() %>"> <img class="default">
									</div>
									<!-- <input name="birthDate" id="birthDate" placeholder="생년월일" disabled> -->
									<div>
										<input type="date" name="birthDate" id="birthDate"
											value="<%= user.getBirthDate() %>" readonly>
									</div>

									<div>
										<input type="password" placeholder="••••••••" readonly><img class="editPencil"
											id="pwEditPencil">
									</div>
								</div>
								...
								<div class="modal-content">
													...
													<div id="pwInputField">
														...
														<div>
															<input id="passwordConfirm" type="password"
																placeholder="비밀번호 확인"><img class="default">
														</div>
													</div>
									...
									<div class="modal-footer">
										<button type="button" class="submit_button add-button" id="hidePasswordModal"
											data-bs-toggle="modal" data-bs-dismiss="modal">취소</button>
										<button type="button" class="submit_button add-button"
											id="time-setting-button">변경</button>
									</div>
								</div>
							</div>
						</div>
					...
					<script>
						...
						const errorMessage = $("#pwErrorMessage");
						...
						$("#add2-add-time-button").on("click", () => location.href = "controller?cmd=updateUserUI");
						...
						async function updateUser(e) {
							e.preventDefault();
							const res = await fetch("controller?cmd=updateNickname&nickname=" + nickname.val());
							...
							if (result) {
								...
								alert("시간이 지나 자동 로그아웃되었습니다. 로그인 후 다시 시도해주세요.");
